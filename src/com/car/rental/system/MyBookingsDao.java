...
public class MyBookingsDao {
	...
	public ArrayList myBookings(double id) {
		System.out.println("id  "+new DecimalFormat("#").format(id));
		String sql = "SELECT bookingdetails.id,userdetails.name,userdetails.email,userdetails.mobile,from_unixtime(bookingdetails.from_date),from_unixtime(bookingdetails.to_date),bookingdetails.pick_address,bookingdetails.drop_address FROM `bookingdetails` LEFT JOIN `userdetails` ON bookingdetails.user_id=userdetails.id WHERE userdetails.id="+id;
