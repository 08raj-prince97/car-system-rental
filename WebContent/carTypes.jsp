	...
	<div id="myAccountPage">
		...
		<table class="table">
			...
			<tbody>
				...
				<c:choose>
					<c:when test="${car_type.size()==0}">
						<p style="text-align: center;">No details found!!!</p>
					</c:when>

					<c:otherwise>
						<c:forEach var="i" begin="0" end="${car_type.size() -1}">

							<tr>
								<th scope="row">${i+1}</th>
								<td>${car_type[i].car_type}</td>
								<td><span onclick="abc(${car_type[i].id})" style="color: red;"data-toggle="modal" data-target="#exampleModal"
									href="<%=request.getContextPath()%>/DeleteCarType?param1=${car_type[i].id}"><i
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
	...
	<script>
	 ...
	 function abcd(){
		 ...
		 location.replace("/Car_Rental_System/DeleteCarType?param1="+window.value);
